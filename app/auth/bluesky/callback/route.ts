...
export async function GET(request: NextRequest) {
  const logPrefix = "[auth/bluesky/callback]";

  const params = extractCallbackParams(request, "bluesky");
  ...
  try {
    ...
    const redirectUri = `${origin}/auth/bluesky/callback`;

    const provider = getProviderForPlatform("bluesky");
    ...
    await clearDeletedTracking(workspaceId, "bluesky");
