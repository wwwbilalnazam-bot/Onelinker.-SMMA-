...
export async function GET(request: NextRequest) {
  const logPrefix = "[auth/threads/callback]";

  const params = extractCallbackParams(request, "threads");
  ...
  try {
    ...
    const provider = getProviderForPlatform("threads");
    ...
    await clearDeletedTracking(workspaceId, "threads");
